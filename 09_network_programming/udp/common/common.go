...
// ErrLength happens when message length is equal or over 256*256 (65536)
...
// MessageContent verifies a message an extracts its contents
func MessageContent(b []byte) ([]byte, error) {
	...
	if length := int(b[4])*256 + int(b[5]); length != n-14 {
